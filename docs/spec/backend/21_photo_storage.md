...
## HealthLog フォームでの写真削除

HealthLog フォームでは、写真が健康記録へ保存済みかどうかで削除タイミングを分ける。

- フォームを開く前から健康記録に紐付いていた写真は、フォーム上で外した時点では削除しない
- 既存写真は、健康記録の更新成功後に `DELETE /api/v1/photos/{photo}` で削除する
- フォームをキャンセルした場合、既存写真の削除予定を破棄する
- フォーム操作中にアップロードした未保存写真は、削除ボタンを押した時点で写真削除 API により削除する
- 健康記録の更新後に写真削除だけが失敗した場合、UI は部分的な失敗として通知する
- 新規アップロード写真を残したままフォームを閉じた場合、その写真は未使用画像 cleanup の対象とする
