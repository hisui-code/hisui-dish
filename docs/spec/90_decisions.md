...
## 2026-03-13: Device の短い通信 retry は通信層へ寄せる

理由:

- `main.py` で retry 分岐まで持つと、常駐ループの責務が広がって読みづらい
- queue の再送管理と、1回の HTTP 呼び出し内の短い retry は役割が異なる

影響:

- `device_settings_client` と `api_sender` の低レベル通信だけで短い retry を行う
- `main.py` は成功 / 失敗の結果だけを見て次の分岐を決める
- queue の `next_retry_at` と `attempt_count` はそのまま維持する
...
## 2026-03-13: 皿あり margin の既定値を少量フード前提へ下げる

理由:

- `tare_weight + 10g` だと、少量だけ残った皿が `NO_BOWL` のままになりやすい
- 実運用では 8g 前後の残量でも皿ありとして扱いたい
- absent 側も少し下げつつ、present より低くしてヒステリシスは維持したい

影響:

- `BOWL_PRESENT_MARGIN_G` の既定値を `5.0g` にする
- `BOWL_ABSENT_MARGIN_G` の既定値を `3.0g` にする
- 皿が軽い状態でも `NO_BOWL` から `IDLE` へ復帰しやすくなる
...
## 2026-03-14: 食事イベント前後の debug capture は device ローカルへ限定保存する

理由:

- `idle_ref` に食前ノイズが入ったかは、確定イベントの `idle_last` だけでは判断できない
- `journalctl` は欲しい時間帯のログが後から取れないことがある
- 常時ファイルへ書き続けるより、イベント前後だけを限定保存した方が SD カード負荷を抑えやすい

影響:

- device は直近3分ぶんの要約サンプルをメモリ上で保持する
- `eat_started` から `eat_finished` `eat_discarded` `eat_aborted` の前後3分だけを `meal_debug` としてローカル保存する
- debug capture は backend へ送らない
- 保存ファイルは3か月を超えたら device 側で自動削除する

## 2026-03-14: `idle_ref` の上方向更新は段階的な食前ノイズ中も凍結する

理由:

- `idle_ref` の長め保留は「現在の `idle_ref` から 4g 以上増えた時」だけだと、移動平均でならされた段階上昇を防げない
- 実機では食前接触ノイズが `184g台 -> 192g台` のように数秒で段階的に上がり、`idle_ref` が途中で追随して過大計上になった
- 食事開始候補に相当する上方向変動中は、食事量算出用の基準を動かさない方が説明しやすい

影響:

- `IDLE` 中に `tracking_baseline` から開始しきい値以上の上方向変動が出ている間は、`idle_ref` の通常更新を止める
- 本当に高い帯へ補充された場合は、待機基準の再設定後に長め安定を満たした時だけ新しい `idle_ref` に昇格する
- 食前接触で `idle_last` が段階的に押し上がるケースを抑えやすくする
