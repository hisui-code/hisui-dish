...
## 2026-07-19: HealthLog の既存写真は記録更新後に削除する

理由:

- 既存写真をフォーム上で外した時点で削除すると、フォームをキャンセルしても写真を復元できない
- 健康記録の更新前に写真を削除すると、更新失敗時に既存の記録から写真だけが失われる
- フォーム操作中にアップロードした未保存写真は、健康記録との紐付けがないため即時削除できる

影響:

- 既存写真はフォーム上で削除予定として保持し、健康記録の更新成功後に写真削除 API へ渡す
- フォームをキャンセルした場合、既存写真の削除予定を破棄する
- フォーム操作中にアップロードした未保存写真は、削除ボタンを押した時点で削除する
- 健康記録の更新後に写真削除が失敗した場合、UI は部分的な失敗として通知する
- フォームを閉じたことで残った未使用写真は、期限付き cleanup の対象とする
